...
export default function AdminAccountsScreen() {
  ...
  const load = async () => { setLoading(true); try { setRows(await fetchAdminRows('jela_accounts', 'id,first_name,last_name,display_name,status,created_at') as Row[]); setError(null); } catch (e) { setError(e instanceof Error ? e.message : 'Could not load accounts.'); } finally { setLoading(false); } };
  useEffect(() => { void load(); }, []);
